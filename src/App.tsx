...
function App() {
  const { feeds, loading: feedsLoading, error: feedsError, refresh, create, rename, remove } = useFeeds();
  ...
  return (
    <main className="flex h-screen flex-col overflow-hidden bg-background text-foreground">
      ...
      >
        ...
        >
          <aside
            className={`border-border bg-card ${feedsCollapsed ? "flex items-start justify-center border-r p-2" : "border-r p-4"}`}
            aria-label="订阅源面板"
          >
          ...
          <section
            className={`flex min-h-0 flex-col border-border bg-background ${articlesCollapsed ? "items-center border-r p-2" : "border-r"}`}
            aria-label="文章列表面板"
          >
