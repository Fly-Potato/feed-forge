...
- `SyncEvent` 是带结构化变体的 Serde 枚举：`rename_all = "camelCase"` 只处理事件名，变体内部的 `job_id` 还必须通过 `rename_all_fields = "camelCase"` 输出为前端契约要求的 `jobId`。
...
- `src-tauri/src/modules/sync/tests.rs` 的 Channel 事件序列化字段名回归测试。
